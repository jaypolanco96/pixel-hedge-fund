...
import { setLeverage } from '$lib/data/blofin';
import { runWithRequestExchangeAuth } from '$lib/server/requestExchangeAuth';
import type { RequestHandler } from './$types';
import type { BloFinSetLeverageBody } from '$lib/data/blofinTypes';
...
export const POST: RequestHandler = async ({ request }) => {
	return runWithRequestExchangeAuth(request, async () => {
		...
		const result = await setLeverage(body as BloFinSetLeverageBody);
		return json(result, {
