...
import { cancelOrder } from '$lib/data/blofin';
import { runWithRequestExchangeAuth } from '$lib/server/requestExchangeAuth';
import type { RequestHandler } from './$types';
import type { BloFinCancelOrderBody } from '$lib/data/blofinTypes';
...
export const POST: RequestHandler = async ({ request }) => {
	return runWithRequestExchangeAuth(request, async () => {
		...
		const result = await cancelOrder(body as BloFinCancelOrderBody);
		return json(result, {
