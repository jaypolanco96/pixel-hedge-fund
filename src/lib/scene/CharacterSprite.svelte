...
>
	...
	<div class="desk-cluster">
		...
		<div class="activity">
			<span>{activity}</span>
			{#if trader}
				<div class="trade-duration" title={tradeDurationEnabled ? undefined : 'Waiting for market data'}>TIME <input type="number" min="0" max="100000" step="1" aria-label={`${trader.name} trading duration in minutes`} value={durationDraft ?? tradeDurationMinutes ?? ''} placeholder="--" disabled={!tradeDurationEnabled} onclick={(event) => event.stopPropagation()} onpointerdown={(event) => event.stopPropagation()} onfocus={startDurationEdit} oninput={(event) => { durationDraft = (event.currentTarget as HTMLInputElement).value; }} onblur={commitTradeDuration} onkeydown={durationKeydown} /><span>m</span>{#if canRelease}<button type="button" class="release" title="Release: close this position and let the trader think again" aria-label={`Release ${trader.name}: close the position and let them think again`} onpointerdown={(event) => { event.stopPropagation(); event.preventDefault(); }} onclick={(event) => { event.stopPropagation(); event.preventDefault(); onRelease(); }}>&times;</button>{:else}<button type="button" class="start" title="Start: put this trader in a position now" aria-label={`Start ${trader.name} trading now`} disabled={!tradeDurationEnabled} onpointerdown={(event) => { event.stopPropagation(); event.preventDefault(); }} onclick={(event) => { event.stopPropagation(); event.preventDefault(); onStart(); }}><i class="play-tri" aria-hidden="true"></i></button>{/if}</div>
			{/if}
		</div>
	</div>
...
<style>
	...
	.trade-duration .start { flex:none; display:flex; align-items:center; justify-content:center; width:11px; height:11px; margin-left:1px; padding:0; color:#eafff0; background:#2e7d4f; border:1px solid #4fd980; cursor:pointer; }
	.trade-duration .start:hover, .trade-duration .start:focus-visible { background:#37974f; outline:1px solid #efc870; }
	.trade-duration .start:disabled { background:#3a3a30; border-color:#5a5a4a; cursor:default; }
	.trade-duration .play-tri { width:0; height:0; border-style:solid; border-width:3px 0 3px 5px; border-color:transparent transparent transparent #eafff0; }
